...
9. **Google マップ** — `[ef_map]` ショートコードを会社概要ページのアクセス欄に配置すると、
   本社所在地の地図と「Google マップで見る」リンクが出力されます。
   埋め込み URL は「外観 → カスタマイズ → 会社情報 → Googleマップ埋め込みURL」で差し替え可能で、
   未設定の場合は本社住所（〒131-0042 東京都墨田区東墨田2-12-20）の埋め込みが既定値になります。
   API キーは不要です（`output=embed` 形式）。
